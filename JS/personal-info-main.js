function addScript(url) {
    document.write("<script =javascript src=./config/config.js></script>");
}
addScript();
//不使用配置文件请注释点上面代码
...
$(function () {
    if (config) {
        ...
        if (config.icon) {
            for (let i = 0; i < config.icon.length; i++) {
                ...
                $(".icon-insert").append(
                    "<a class=\"my-tooltip\" href=\"" + url + "\" title=\"visit my social media\">\n" +
                    "    <img src=\"" + img + "\" alt=\"\">\n" +
    ...
    // ---- 座右铭字缓出效果 -----
    var text = "Life every day like it's your last.";
    ...
    //     mottoArrs = data;
    // })
    var e1 = setInterval(insWords, 100);
    var e2 = 0;
